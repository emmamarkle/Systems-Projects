...
populate (vmsim_addr_t array, unsigned int length) {
  ...
  for (uint64_t i = 0; i < length; i += 1) {
    ...
    vmsim_write(&i, addr, sizeof(i)); // writes 8 bytes from i to a simulated address (addr)
    if (debug) {
      uint64_t value;
      vmsim_read(&value, addr, sizeof(value)); // will read 8 bytes stored at the simulated address (addr) into value
      fprintf(stderr,
...
traverse (vmsim_addr_t array, unsigned int length) {
  ...
  for (uint64_t i = 0; i < length; i += 1) {
    ...
    vmsim_addr_t addr = array + (i * sizeof(i));
    uint64_t current;
    vmsim_read(&current, addr, sizeof(current)); // will read 8 bytes from the simulated address (addr) into current
    sum += current;
    vmsim_write(&sum, addr, sizeof(sum)); // will write 8 bytes from sum to the simulated address (addr)
    if (debug) {
      fprintf(stderr,
...
go (unsigned int length, unsigned int iterations) {
  ...
  vmsim_read(&value, addr, sizeof(value)); // will read 8 bytes from the simulated address (addr) into value
  printf("%ld\n", value);
