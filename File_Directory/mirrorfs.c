...
{
	// initializing variables that will be used later in the code
	int fd; // variable for file descriptor
	int res; // result
	int i; // variable for a for loop to iterate over
	char temp_buf[size];

	// helpful debug statement
	fprintf(stderr, "DEBUG: Reading from %s\n", path);

	(void) fi; // cast to void so we dont use
	path = prepend_storage_dir(storage_path, path); // gets path to file
	//printf("path: %p\n", path);
	fd = open(path, O_RDONLY); // opens file as read only
	//printf("fd: %u\n", fd);

	// could not open file so return error
	if (fd == -1) 
		return -errno;

	// from desired file (fd), read and store read info in temp buffer of size (size) from the offset point (offset)
	res = pread(fd, temp_buf, size, offset); // reads file from desired offset
	// return error if the read is unsuccessful
	if (res == -1)
		res = -errno;
	...
	close(fd); // close file
	return res; // return result (# of bytes read)
}
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	int res; // result
	int i; // var for a for loop 
	char temp_buf[size]; // temp buffer

	// debug statement
	fprintf(stderr, "DEBUG: Writing to %s\n", path);

	(void) fi; // cast to void so we dont use later in code
	path = prepend_storage_dir(storage_path, path); // get path to file
	fd = open(path, O_WRONLY); // open file as write only

	// could not open file, return error
	if (fd == -1)
		return -errno;

	// copies chars from arg *buf (taken in arg) into temp buffer
	for (i = 0; i < size; i += 1) {
	  temp_buf[i] = buf[i];
	}

	// writing from temp buffer to fd
	...
	close(fd); // close file 
	return res; // return # of bytes written
}
